...
export const Comment = ({ post, showCommentsLoading, setIsAction, isAction }) => {
    ...
    const commentFormRender = (status) => {
        ...
        return <CommentForm post={post} setIsAction={setIsAction} isAction={isAction} />
    }
            ...
            <div className='relative flex flex-col gap-4 max-sm:ml-4 ml-10 duration-300 ease-in-out transform'>
                {showCommentsLoading
                    ...
                    : post?.support_post_comment?.map((comment, _index) => (
                        <div key={comment?.xsl} className='bg-white first:mt-6 mr-3 relative border-2 border-gray-300 rounded-xl mb-4 shadow-[0_2px_10px_3px_rgba(0,0,0,0.1)] transition-transform duration-300 transform hover:scale-105'>
                            ...
                            <div className='flex px-4 mt-2'>
                                ...
                                <div className='ml-[23%] max-sm:ml-[32%]'>
                                    <h4 className='text-lg max-sm:text-[12px] font-bold text-[#050506]'>{comment?.xreply_name}</h4>
                                    <p className='text-sm max-sm:text-[9px] text-gray-900'>{comment?.xreply_designation}, Mentors IT</p>
                                </div>
                            </div>
                            ...
                            <div className="px-4 py-5">
                                {comment?.xcomment?.length > 300 ? (
                                    <div>
                                        <div id="quilljs-editor" dangerouslySetInnerHTML={{ __html: expandedQuestions[comment?.xsl || ""] ? comment?.xcomment : comment?.xcomment?.slice(0, 300) + "..." }} />
                                        <button onClick={() => toggleShowMore(String(comment?.xsl))} className="text-blue-500 text-[12px] md:text-[14px] hover:underline">
                                            {expandedQuestions[comment?.xsl || ""] ? "Show Less" : "Show More"}
                                        </button>
                                    </div>
                                ) : (
                                    <div id="quilljs-editor" dangerouslySetInnerHTML={{ __html: comment?.xcomment }} />
                                )}
                            </div>
                            <div className='text-sm max-sm:text-[10px] py-2 px-6 bg-red-400 text-white flex justify-end items-center rounded-xl'>{timeFormat(comment?.ztime)}</div>
                        </div>
