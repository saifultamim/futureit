...
//                     {homework?.xdate && dateFormat(homework?.xdate)}
//                   </div>
//                   <div
//                     className={`text-sm ${
...
//                     {homework?.xduedate && dateFormat(homework?.xduedate)}
//                     <div
...
//                     {homework?.homework_submit?.[0]?.xmarks}
//                   </div>
//                 </TableCell>
//                 <TableCell>
//                   <span
...
//                           __html:
//                             homework?.xdescription ??
//                             "No additional details available.",
//                         }}
//                       />
//                       {isOverdue && !isSubmitted ? (
//                         <p className="mt-2 text-red-600 font-medium">
//                           Note: The submission deadline for this homework has
//                           passed. You can no longer submit your work.
//                         </p>
//                       ) : dueDateStatus.isSameDay && !isSubmitted ? (
//                         <p className="mt-2 text-orange-600 font-medium">
//                           Note: This homework is due today. Please submit before
//                           midnight.
//                         </p>
//                       ) : null}
//                     </div>
    ...
    <Table>
      <TableHeader>
        <TableRow className="bg-gray-50 border-b">
          <TableHead className="font-semibold uppercase">#</TableHead>
          <TableHead className="font-semibold uppercase">Date</TableHead>
          <TableHead className="font-semibold uppercase">Question Detail</TableHead>
          <TableHead className="font-semibold uppercase">Mark Info.</TableHead>
          <TableHead className="font-semibold uppercase">Submission Status</TableHead>
          <TableHead className="font-semibold uppercase">PDF</TableHead>
          <TableHead className="font-semibold uppercase">Actions</TableHead>
        </TableRow>
      </TableHeader>
          ...
          return (
            <React.Fragment key={item?.xquesid || index}>
              <TableRow className="hover:bg-gray-50">
                ...
                <TableCell className="text-gray-600">
                  ...
                  <div className={`text-sm ${isOverdue ? "text-red-600 font-medium" : ""}`}>
                    ...
                    <div className={`text-sm font-medium mt-1 ${
                      ...
                      dueDateStatus.isSameDay ? "text-orange-600" :
                      dueDateStatus.daysRemaining <= 2 ? "text-yellow-600" :
                      "text-green-600"
                    }`}>
                      {dueDateStatus?.message}
                    </div>
                ...
                <TableCell className="text-gray-600 space-y-1">
                  <div className="text-sm">
                    <span className="font-semibold">Ques. Mark:</span> {item?.xmarks}
                  </div>
                  <div className="text-sm">
                    <span className="font-semibold">Your Mark:</span>{" "}
                ...
                <TableCell>
                  <span className={`inline-flex items-center justify-center px-3 py-1 text-sm font-medium rounded-full ${
                    ...
                    dueDateStatus.isSameDay ? "bg-orange-100 text-orange-700" :
                    "bg-yellow-100 text-yellow-700"
                  }`}>
                    {isSubmitted ? "Submitted" :
                     isOverdue ? "Overdue" :
                     dueDateStatus.isSameDay ? "Due Today" :
                     "Not Submitted"}
                  </span>
                </TableCell>
                <TableCell className="text-left space-y-2">
                  {item?.xfile_name || item?.homework_submit?.[0]?.xfile_name ? (
                    <>
                      {item?.xfile_name && (
                        <div>
                          <a
              ...
              {expandedRow === index && (
                <TableRow>
                  <TableCell colSpan={8} className="bg-gray-50">
                    <div className="px-4 text-sm text-gray-600">
                      <h3 className="font-bold text-gray-800">{item?.xtitle}</h3>
                      <p
                        dangerouslySetInnerHTML={{
                          __html: item?.xdescription ?? "No additional details available.",
                        }}
                      />
                      {isOverdue && !isSubmitted ? (
                        <p className="mt-2 text-red-600 font-medium">
