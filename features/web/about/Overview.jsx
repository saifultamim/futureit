        ...
        <ul className="grid grid-cols-2 md:grid-cols-4 gap-4 list-inside text-[#605f62]">
            {
                courses?.courseName?.map((course,idx)=>(
                   <li key={idx} className={`shadow-lg py-3 px-2 md:px-8 border text-sm self-center md:text-base font-medium cursor-pointer rounded flex item-center justify-center ${idx==0 ? 'bg-secondary' : 'hover:bg-secondary text-[#605f62]'}  text-white transform duration-300  hover:text-white`}>
            {course}
          </li>
                ))
            }
