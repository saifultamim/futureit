...
export const Posts = ({ setIsAction, isAction }) => {
  ...
  const postsRender = (_posts = []) => {
    ...
    return _posts?.map((post, index) => (
      <div
        key={post.xsl}
        ref={index === _posts.length - 1 ? lastPostRef : null}
      >
        <div
          key={index}
          className={`bg-slate-50 relative border-2 border-gray-300 rounded-xl mb-4 shadow-[0_2px_10px_3px_rgba(0,0,0,0.1)]`}
        >
          {/* Question status */}
          {post?.xstatus == "Solved" && (
          ...
          <div className="flex px-4">
            ...
            <div className="ml-[23%] max-sm:ml-[30%] mt-2">
              <h4 className="text-lg font-bold text-[#050506] max-sm:text-[12px]">
                {post?.edustudent?.xstuname}
              </h4>
              <p className="text-[12px] text-gray-900 max-sm:text-[9px]">
                {" "}
                {"Student, Mentors IT"}
              </p>
              <p className="text-[12px] text-gray-900 max-sm:text-[9px]">
                Student ID: {post?.xstudent}
              </p>
            </div>
          </div>
          {/* content  */}
          <div className="px-4 py-5">
            {post?.xmessage?.length > 300 ? (
              <div>
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: expendedPosts[post.xsl || ""]
                      ? post.xmessage
                      : post.xmessage.slice(0, 300) + "...",
                  }}
                />
                <button
                  onClick={() => toggleShowMore(post?.xsl)}
              ...
              <div
                id="quilljs-editor"
                dangerouslySetInnerHTML={{ __html: post.xmessage }}
              />
            )}
          </div>
          {/* action section  */}
          <div
            className={`${
              post?.xstatus == POST_STATUS.PENDING
                ? "bg-red-400/40"
                : "bg-red-400"
            } text-sm max-sm:text-[10px] py-2 px-6  text-white flex justify-between items-center rounded-xl`}
          >
            <div
              className={`${
                post?.xstatus == POST_STATUS.PENDING ? "" : "cursor-pointer"
              } w-fit flex items-center gap-1`}
              onClick={() => handleClickComment(post, index)}
            >
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1 text-sm">
                  <span>Comments</span>{" "}
                  <span className="ml-1">
                    ({post?.support_post_comment?.length || 0})
                  </span>
                </div>
              </div>
            </div>
          ...
          {/* comment section  */}
          {commentIndex === index && showComments !== null && (
            <Comment
              setIsAction={setIsAction}
              isAction={isAction}
              post={post}
              showCommentsLoading={showCommentsLoading}
            />
          )}
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="max-w-[600px] mx-auto mt-14 flex flex-col gap-6">
        {postsRender(posts)}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      {Array.from({ length: 3 }).map((_, index) => (
        <div
          key={index}
          className={`bg-slate-50 relative border-2 border-gray-300 rounded-xl mb-4 shadow-[0_2px_10px_3px_rgba(0,0,0,0.1)] animate-pulse`}
        >
          {/* Question status */}
